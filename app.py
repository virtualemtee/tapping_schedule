...
if uploaded_file is not None:
    ...
    if 'CELL' in data.columns and 'Si' in data.columns and 'Fe' in data.columns:
        ...
        for index, row in filtered_data.iterrows():
            ...
            if individual_grade in ['1535', '2050'] and cell_id not in used_cells:
                ...
                if best_pairing is not None:
                    suggested_tapping_schedule.append({
                        "Cell_A": cell_id,
                        "Cell_B": best_pairing,
        ...
        # Display the "Suggested Tapping Schedule" table
        st.subheader("Suggested Tapping Schedule")
        if suggested_tapping_schedule:
            st.dataframe(pd.DataFrame(suggested_tapping_schedule))

        # Display "Standalone Cells" if any exist
        if remaining_cells:
        ...
        with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
            # Write "Suggested Tapping Schedule" data to the Excel file
            pd.DataFrame(suggested_tapping_schedule).to_excel(writer, sheet_name="Suggested Tapping Schedule", index=False)
            
            # Write "Standalone Cells" data only if there are unpaired cells
            if remaining_cells:
                pd.DataFrame(remaining_cells).to_excel(writer, sheet_name="Standalone Cells", index=False)
